...
namespace WMS
{
    public partial class ZaprimiRobuForm : Form
    {
        PocetnaForm? _glavniObrazac = null;
        ProductEntity? odabraniProizvod = null;
        public ZaprimiRobuForm(PocetnaForm glavniObrazac)
        {
            _glavniObrazac = glavniObrazac;
            InitializeComponent();
        }

        private void ZaprimiRobuForm_Load(object sender, EventArgs e)
        {
        ...
        private void ZaprimiBtn_Click(object sender, EventArgs e)
        {
            using (var context = new DucanPlusDbContext())
            {
                if (context.Products != null && context.Products.Count() > 0)
                {
                    ...
                    context.SaveChanges();
                    PovijestLb.Items.Add($"Zaprimljeno {kolicina} komada proizvoda {proizvod.Name}.");
